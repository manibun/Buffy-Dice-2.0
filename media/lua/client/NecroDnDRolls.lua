...
function DnDRules.setLocalSandboxVars()
	...
	if dicebudgetmax == nil then --there be dragons here
		dicebudgetmax = 20; --default to 20
	end
end
DiceRolling = false;
local function get_armorBonus()
	armorBonus = 0
...
function ISSkillProgressBar:onMouseDown(x, y)
	...
	if DiceRolling and not isCtrlKeyDown() and not isShiftKeyDown() then
		...
		self.perk == Perks.DnDStr_Accuracy or
		...
		self.perk == Perks.DnDWillpower or
		...
		if self.perk == Perks.DnDStr_Defense then
			...
			if armorBonus > 3 then
				armorBonus = 3
			end
			result = result + armorBonus
			modifier = ": " .. realroll .. "+" .. perkLevel .. "+" .. armorBonus .. "= ";
		end
		if self.perk == Perks.DnDStr_Evasion then
			armorBonus = 0
			for i=0,getPlayer():getWornItems():size()-1 do
				local item = getPlayer():getWornItems():get(i):getItem();
				if item and not instanceof(item, "InventoryContainer") then
	...
	if total_dice_skills() < dicebudgetmax and not DiceRolling then
		if self.perk == Perks.DnDStr_Strength or
		self.perk == Perks.DnDStr_1hMelee or
		self.perk == Perks.DnDStr_Defense or
		self.perk == Perks.DnDStr_Evasion or
		...
		if string.match(tostring(self.perk:getType()), "DnDStr_") then
			...
			self.char:getPerkLevel(Perks.DnDStr_Accuracy) +
		...
		if total_dice_skills() == dicebudgetmax then
			DiceRolling = true;
			return
		end
	elseif total_dice_skills() == dicebudgetmax then
...
function ISCharacterInfo:render()
	...
	if DiceRolling then
		...
		y = y + 20
		self:drawText("Armor Bonus: "..armorBonus, left + 5, y, 1, 1, 1, 1, UIFont.Small);
		y = y + 5
	else
		self:drawText("Click a _Dice skill to level it up.", left + 5, y, 1, 1, 1, 1, UIFont.Small);
		y = y + 25;
		self:drawText("Assigned: " .. total_dice_skills() .. "/"..dicebudgetmax,left + 5, y, 1, 1, 1, 1, UIFont.Small);
		y = y + 5;
    end
...
function reset_skills()
	local playerchar = getPlayer()
	DiceRolling = false;
	local dicetotals = total_dice_skills()
	if dicetotals > 0 then
		...
		playerchar:LoseLevel(Perks.DnDStr_Accuracy);
		playerchar:getXp():setXPToLevel(Perks.DnDDex_Accuracy, playerchar:getPerkLevel(Perks.DnDStr_Accuracy))
...
function total_dice_skills()
	...
	playerchar:getPerkLevel(Perks.DnDStr_Accuracy) + 
